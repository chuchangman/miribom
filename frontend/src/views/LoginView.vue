...
<template>
  ...
  <form class="login-form" @submit.prevent="handleLogin">
    <div>
      ...
      <input type="email" id="email" v-model.trim="email" @blur="validateEmail" @focus="removeEmailError"/>
      <p class="error-message" v-if="emailErrorMessage">{{ emailErrorMessage }}</p>
      <label for="password">비밀번호:</label>
      <input type="password" id="password" v-model.trim="password" @blur="validatePassword" @focus="removePasswordError"/>
...
<style scoped>
  ...
  #login-btn:hover {  background-color: #007bb5;
  }
  .error-message {
    margin-top: -10px;
    color: red;
